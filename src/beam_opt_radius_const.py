...
def opt_r(material, def_max, thickness_min, P, Lx, Ly, R0):
    E = material["E"]
    ...
    def shear_constr(x, grad):
        ...
        return np.pi * (Mx * R0 * (R0**2 - x[0]**2) + \
                        P * (R0**4 - x[0]**4) - \
                        0.5 * shear_str * np.pi * (R0**4 - x[0]**4) * (R0**2 - x[0]**2))

    def tensile_constr(x, grad):
        if grad.size > 0:
            grad[0] = yield_str * np.pi * 4 * x[0]**3
        return 4 * My * R0 - yield_str * np.pi * (R0**4 - x[0]**4)

    def def_constr(x, grad):
        if grad.size > 0:
            grad[0] = def_max * 12 * E * np.pi * x[0]**3
        return 4 * P * Lx**3 - def_max * 3 * E * np.pi * (R0**4 - x[0]**4)

    opt = nlopt.opt(nlopt.LD_MMA, 1)
    opt.set_lower_bounds([0.01])
    opt.set_upper_bounds([R0 - thickness_min])
